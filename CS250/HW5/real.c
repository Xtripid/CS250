...
CACHE *cache_sim;
int lru_count = 0;
char main_mem[16777216][2];

void mem_create() {
  for(int i = 0; i < 16777216; i++) {
    strcpy(main_mem[i], "00");
  }
}
...
void cache_create(int csize, int associativity, int bsize) {
  ...
  for (int i = 0; i < cache_sim->num_of_lines; i++) {
    ...
    for(int j = 0; j < cache_sim->set_size; j++) {
      ...
      curr_block->data = (char**)malloc(2* sizeof(char*) * cache_sim->block_size);
      for(int k = 0; k < (sizeof(curr_block->data)/sizeof(curr_block->data[0])); k++) {
        curr_block->data[k] = (char *)malloc((sizeof(char) + sizeof(char)) * (sizeof(curr_block->data)));
      }
    }
  }
}
  ...
  if(!found) {
    ...
    if(lru_block->dirty == 1) {
      ...
      for(int l = placeInMem; l < placeInMem + y; l++) {
        strcpy(main_mem[l], lru_block->data[z]);
        z++;
      }
    }
    int x = 0;
    for(int m = address; m < address + y; m++) {
      strcpy(lru_block->data[x], main_mem[m]);
...
void cache_load(int address, int size_access) {
  ...
  if(!found) {
    ...
    char byte[toGet+1];
    for (int p = 0; p < size_access; p++) {
      strcat(byte,lru_block->data[temp]);
      temp++;
    }
    byte[toGet] = '\0';
    found = 1;
